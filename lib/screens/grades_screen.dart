...
class _GradesScreenState extends State<GradesScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (_student == null) {
        setState(() {
          _error = 'No student record found for $email.';
          _isLoading = false;
        });
        return;
      }

      // Fetch all collections for the student
      final sid = _student!.id;
      final collections = await Future.wait([
        _firestoreService.getAttendance(sid).first,
        _firestoreService.getQuizzes(sid).first,
        _firestoreService.getExams(sid).first,
        _firestoreService.getActivities(sid).first,
        _firestoreService.getOralRecitations(sid).first,
        _firestoreService.getProjects(sid).first,
      ]);
  ...
  Future<void> _showSimulateDialog(
    ...
    final surfaceColor = isDark ? const Color(0xFF1A1230) : Colors.white;
          ...
          children: [
            ...
            Column(
              ...
              children: [
                ...
                Text(title, style: TextStyle(color: isDark ? Colors.white54 : Colors.black54, fontSize: 13, fontWeight: FontWeight.w600)),
              ],
            ),
          ],
        ),
  ...
  Widget build(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final bgColor = isDark ? const Color(0xFF0F0A1E) : const Color(0xFFF8F9FE);
    const accent = Color(0xFF8B5CF6);

    if (_isLoading) {
      return Scaffold(backgroundColor: bgColor, body: const Center(child: CircularProgressIndicator(color: accent)));
    }

    if (_error != null) {
      return Scaffold(
        backgroundColor: bgColor,
        body: Center(child: Text(_error!, style: TextStyle(color: isDark ? Colors.white : Colors.black))),
      );
    }
                  ...
                  _buildGridCard(
                    ...
                    onTap: () => _showDetailsModal(context, 'Projects', Column(
                      children: _projects.map((p) => ListTile(
                        ...
                        trailing: Text('+${p.score} pts', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.teal)),
                      )).toList(),
